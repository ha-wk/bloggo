...
func (p *Post) FindAllPosts(db *gorm.DB) (*[]Post, error) {
	...
	if len(posts) > 0 {
		for i, _ := range posts {
			...
			//Take function returns and insert a record that matches with given condition
...
func (p *Post) DeleteAPost(db *gorm.DB, pid uint64, uid uint32) (int64, error) {
	var err error
	err = db.Debug().Model(&Post{}).Where("id=? and author_id=?", pid, uid).Take(&Post{}).Delete(&Post{}).Error
	if err != nil {
...
//CONSIDER BELOW FUNCTION IF ABOVE DOES'NT WORKS!
/*func (p *Post) DeleteAPost(db *gorm.DB, pid uint64, uid uint32) (int64, error) {
